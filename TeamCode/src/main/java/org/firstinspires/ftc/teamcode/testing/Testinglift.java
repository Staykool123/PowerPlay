...
public class Testinglift {
    public DcMotorEx liftMotor; //for better PID of run to position

    protected HardwareMap hwMap;
    //protected TeleBug teleBug;

    private static final double TICKS_PER_MOTOR_REV     = 117; //Andymark motor on rev 20 gear box
    private static final double WHEEL_DIAMETER_INCHES   = 1.25
            ;     // For figuring circumference
    public static final double TICKS_PER_INCH = TICKS_PER_MOTOR_REV / (WHEEL_DIAMETER_INCHES * Math.PI);

    private static final int POSITIONING_TOLERANCE = 15; //The amount of ticks the DriveByInch method should be allowed to deviate by
    public static int BOTTOM = 0; //position at the bottom
    private static final int TOP = 1000; //position at the top
    public int targetPos;

    public Testinglift (HardwareMap hwMap){
        this.hwMap = hwMap;
    }
